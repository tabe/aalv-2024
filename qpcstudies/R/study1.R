.study1_simulation_length <- 500 # the number of time points in a simulated sample (i.e. epoch)

.study1_sample_size <- 128 # the number of simulated samples (i.e. epochs)

.study1_ntp <- .study1_simulation_length * .study1_sample_size # the total number of time points in given input time series

.study1_ntp_in_window <- 500 # the number of time points in a window

.study1_nw <- 255 # the total number of windows

stopifnot((.study1_ntp_in_window/2) * (.study1_nw+1) <= .study1_ntp)
...
study1_sim <- function(args) {
    ...
    i1 <- function(x, p) {2 * asin(sin(2 * pi * param$Fcoef1 * x + p))}
    i2 <- function(x, p) {ifelse(cos(2 * pi * param$Fcoef2 * x + p) >= 0, -1, 1)}
    ...
    sample_tc <- function() {
        Map(function(f) {f(seq_len(.study1_simulation_length) / .sampling_rate)}, Map(tc, seq_len(.study1_sample_size)))
    }

    xi3_data_frame <- function(y) {
        do.call(cbind, Map(function(k) {y[[k]]$xi3}, seq_len(.study1_sample_size)))
    }

    s3_data_frame <- function(y) {
        do.call(cbind, Map(function(k) {y[[k]]$s3}, seq_len(.study1_sample_size)))
    }

    c1_data_frame <- function(y) {
        do.call(cbind, Map(function(k) {y[[k]]$c1}, seq_len(.study1_sample_size)))
    }

    c2_data_frame <- function(y) {
        do.call(cbind, Map(function(k) {y[[k]]$c2}, seq_len(.study1_sample_size)))
    }

    c3_data_frame <- function(y) {
        do.call(cbind, Map(function(k) {y[[k]]$c3}, seq_len(.study1_sample_size)))
    }
...
study1_bqpc <- function(args) {
    ...
    z <- long_data$c3
    bqpc <- .bqpc(x, y, z,
                  nw = .study1_nw, ntp_in_window = .study1_ntp_in_window,
                  bin_width = .bin_width,
...
#' @importFrom magrittr %>%
#' @export
study1_fig_xbcmap <- function(args) {
...
#' @export
study1_fig_mvgc_line <- function(args) {
    .plot_mvgc(args[1], geom_line)
}

#' @export
study1_fig_mvgc_area <- function(args) {
    .plot_mvgc(args[1], geom_area)
}

#' @export
study1_ugctest_ic <- function(args) {
    if (length(args) < 3)
        stop("missing arguments")
...
#' @export
study1_tbl_ratio <- function(args) {
    if (length(args) < 4)
        stop("missing arguments")
...
#' @importFrom dplyr mutate
#' @export
study1_fig_ratio <- function(args) {
    if (length(args) < 2)
        stop("missing arguments")
...
#' @export
study1_fig7_line <- function(args) {
    .study1_fig7(args, geom_line)
}

#' @export
study1_fig7_area <- function(args) {
    .study1_fig7(args, geom_area)
}
