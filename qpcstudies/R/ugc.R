## Modified Granger.inference.unconditional from grangers
...
requireNamespace("tseries")
  ...
  cause_x.to.y_bp[w, ] <- G.xy$Unconditional_causality_x.to.y
    ...
    p_x <- top_bp_y.to.x[test_stationarity == 0]
    p_y <- top_bp_x.to.y[test_stationarity == 0]
    ...
    GG <- list(freq.good, n, nboots, conf, stat_yes, non_stationarity_rate, mean(delay_bp[test_stationarity == 0]), q_x, q_y, 
        freq.good[signif_x],freq.good[signif_y],q_max_x,q_max_y, freq.good[signif_max_x], freq.good[signif_max_y], p_x, p_y)
